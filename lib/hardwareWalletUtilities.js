...
export const signTX = async (bcoinApp, client, accountPath, proposalMTX, walletId) => {
  ...
  for (let i = 0; i < mtx.inputs.length; i++) {
    ...
    console.log(`getCoin(${walletId},${hash},${index})\n`)
    console.log(coin);

    console.log(`getTx(${walletId},${hash})\n`)
    console.log(txDetails);
    txDetails.inputs[i].prevout = { hash: coin.hash, index: coin.index, script: coin.script };

    //const transaction = new MTX(txDetails).toRaw();
    //console.log(new MTX(txDetails));
    ...
    console.log(`LedgerTXInput using index: ${fullPath}`);
