...
export const safeEval = (fn, fallback, context = '') => {
  try {
    const response = fn();
...
export const buildSingleColumnTable = (data = [], title = '') => {
  // if data gets passed in as null the empty array substitution
  // doesn't happen. The below will catch this situation
  if (!data) data = [];
...
export const parseToken = joinCosigners => {
  let joinToken = null;
  // find the first token and assign it to joinToken
