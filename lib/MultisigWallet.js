...
class MultisigWallet extends Component {
  ...
  render() {
    ...
    const _proposals = safeEval(() => proposals[selectedMultisigWallet], []);

    // generate table data
    const multisigWalletTableData = buildSingleRowTable(multisigWallets, 'Name');
    const _selectedProposal = _proposals.find(p => p.name === selectedProposal) || {};
    // data to render about proposal
    const { approvals, author, rejections } = _selectedProposal;
    const { tx = {} } = proposalMTX;
    const _selectedProposalInfo =
      (!!selectedProposal)
      ? (<div>
    ...
    ? (<div>
        ...
        <div className="row">
          <div className="col-sm">
            <Table
              ...
              tableData={buildSingleRowTable(_proposals.map(p => p.name), 'Name')}
        ...
        <div className="row">
          <div className="col-sm">
            {/* TODO: clean up _selectedProposal */}
            <Button
              type="action"
              style={styles.wideButton}
              onClick={() => handleCreate({
                walletId: selectedMultisigWallet,
                proposalId: _selectedProposal.name
              }, 'multisigApprove')}
            >
              Approve
            </Button>
...
const mapStateToProps = (state, otherProps) => {
  return {
    ...
    multisigInfo: state.wallets.multisigInfo,
    proposals: state.wallets.proposals,
    proposalMTXs: state.wallets.proposalMTXs,
    multisigInfo: state.wallets.multisigInfo
  }
}
