...
export const mapComponentDispatch = {
  Panel: (dispatch, map) =>
    Object.assign(map, {
      ...
      updateTextField : async (field, value) => dispatch(updateTextField(field, value)),
    })
};
...
export const reduceWallets = (state, action) => {
  ...
  switch (type) {
    ...
    case SET_MULTISIG_WALLET_INFO:
      newState = safeSet(newState, `multisigInfo.${walletId}`, multisigWalletInfo);
      return newState;
    case SET_MULTISIG_WALLET_PROPOSALS:
      newState = safeSet(newState, `proposals.${walletId}`, proposals);
      return newState;
    case SET_MULTISIG_PROPOSAL_MTX:
      newState = safeSet(newState, `proposalMTXs.${walletId}.${proposalId}`, mtx);
      return newState;
    default:
      return state;
  }
...
export const reducePlugins = (state, action) => {
  ...
  switch (type) {
    ...
    case USER_SELECT_MULTISIG_WALLET:
      newState = safeSet(newState, `${PLUGIN_NAMESPACE}.selectedMultisigWallet`, walletId);
      return newState;
    case USER_SELECT_MULTISIG_PROPOSAL:
      newState = safeSet(newState, `${PLUGIN_NAMESPACE}.selectedProposal`, proposalId);
      return newState;
    case USER_SELECT_XPUB:
      newState = safeSet(newState, `${PLUGIN_NAMESPACE}.selectedXPUB`, selectedXPUB);
      return newState;
    case UPDATE_TEXT_FIELD:
      newState = safeSet(newState, `${PLUGIN_NAMESPACE}.textFields.${field}`, { value, valid });
      return newState;
    default:
      return state;
  }
