...
class SelectWallet extends Component {
  ...
  render() {
    ...
    const transactionHistory = safeEval(() => history[selectedWallet][selectedAccount], []);
    const txnHistory = this.parseTxnHistory(transactionHistory);
    const walletTableData = buildSingleRowTable(wallets, 'Wallets');
    const accountTableData = buildSingleRowTable(selectedAccounts, 'Accounts');
      ...
      <div className="container">
        ...
        <div className="row" styles={styles.componentContainer}>
          ...
          <div className="col-sm">
            ...
            <Text type="p">{`Balance: ${walletBalance} BTC`}</Text>
          </div>
          <div className="col-sm">
            <Header type="h4">Account Balance</Header>
            <Text type="p">{`Balance: ${accountBalance} BTC`}</Text>
          </div>
        </div>
        {/* SEND SECTION */}
        <div className="row" styles={styles.componentContainer}>
          ...
          <div className="col-sm">
            ...
            <QRCode containerStyle={styles.qrCodeContainer} text={receiveAddress} />
            <Button
              type="action"
              style={styles.wideButton}
...
const mapStateToProps = (state, otherProps) => {
  ...
  const receiveAddress = safeEval(() => state.wallets.accountInfo[pluginState.selectedWallet][pluginState.selectedAccount].receiveAddress, '');
  const selectedAccounts = safeEval(() => accounts[pluginState.selectedWallet], []);
  const info = state.wallets.info;
  const balance = state.wallets.balance;

  const walletBalance = safeEval(() => new Amount(info[pluginState.selectedWallet].balance.confirmed).toBTC(), '');
  const accountBalance = safeEval(() => new Amount(balance[pluginState.selectedWallet][pluginState.selectedAccount].confirmed).toBTC(), '');
